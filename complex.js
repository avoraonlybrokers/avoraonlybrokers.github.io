...
function renderTextBlock(blockId, textId, value) {
  ...
  // Красивые заголовки для блоков
  const blockMap = {
    "block-about": "О проекте",
    "block-format": "Формат проекта",
    "block-payment": "Схема рассрочки",
    "block-extra": "Дополнительная информация"
  };
  
  const title = blockMap[blockId] || "";
  const titleHtml = title ? `<h2 class="block-title">${title}</h2>` : "";
  
  document.getElementById(textId).innerHTML = titleHtml + `<div class="block-text">${avoraEscapeHtml(value)}</div>`;
}
...
function renderSummary(complex) {
  ...
  document.getElementById("complex-summary").innerHTML = `
    <h1 class="complex-title">${avoraEscapeHtml(name)}</h1>
    <div class="complex-meta">
      <div class="complex-location">
        <i data-lucide="map-pin" width="16" height="16"></i>
        <span>${avoraEscapeHtml(complex.city)}, ${avoraEscapeHtml(complex.country)}</span>
      </div>
      ${priceFrom ? `<div class="complex-price"><span data-i18n="price_from"></span> ${priceFrom}</div>` : ""}
      ${complex.handover_date ? `<div class="complex-handover"><i data-lucide="calendar-clock" width="14" height="14"></i> <span data-i18n="handover"></span>: ${avoraEscapeHtml(complex.handover_date)}</div>` : ""}
      ${yieldText ? `<div class="complex-yield"><i data-lucide="trending-up" width="14" height="14"></i> <span data-i18n="yield"></span>: ${avoraEscapeHtml(yieldText)}</div>` : ""}
    </div>
  `;
}
